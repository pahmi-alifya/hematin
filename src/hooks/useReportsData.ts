...
function buildDonutData(transactions: Transaction[], type: TransactionType): DonutDatum[] {
...
export function useReportsData(transactions: Transaction[]) {
  ...
  const donutData = useMemo(() => buildDonutData(filteredTx, 'expense'), [filteredTx])
  const incomeDonutData = useMemo(() => buildDonutData(filteredTx, 'income'), [filteredTx])
  const savingDonutData = useMemo(() => buildDonutData(filteredTx, 'saving'), [filteredTx])
