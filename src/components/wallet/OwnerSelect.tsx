...
export function OwnerSelect({ options, value, onChange, placeholder }: OwnerSelectProps) {
        ...
        {open && (
          ...
          >
            {options.map((opt) => (
              ...
              >
                <span className="truncate">
                  {opt.label} <span className="text-xs text-slate-400 dark:text-slate-500">· {opt.roleLabel}</span>
                </span>
                {opt.id === value && <Check className="w-3.5 h-3.5 text-sky-500 shrink-0" />}
              </button>
            ))}
          </motion.div>
        )}
